...
exports.smartLinkUtlis = void 0;
const fs = require("fs");
const aws_1 = require("../../helper/aws");
const uuid_1 = require("uuid");
const path = require("path");
const child_process_1 = require("child_process");
const axios = require('axios');
const https = require('https');
class smartLinkUtlis {
    healthcheck() {
        try {
            return {
                "status_code": 200,
    ...
    updateSmartLink(org_id, id, name) {
        try {
            let writeResponse = false;
            const clients = this.readClientsDetail();
            const allOrganizationSmartLinkNodes = this.readSmartLinksDetail();
    ...
    createConnection(org_id, id, connections) {
        try {
            const clients = this.readClientsDetail();
            const allOrganizationSmartLinkNodes = this.readSmartLinksDetail();
            let writeResponse = false;
                    ...
                    allOrganizationSmartLinkNodes.find((organizationSmartLinkNodes) => {
                        organizationSmartLinkNodes.smartLink.find((smartLink) => {
                            if (id == smartLink.id) {
                                ...
                                smartLink.connections.dicom.push(Object.assign({ "conn_id": connectionId }, connections.dicom[0]));
                                smartLink.last_updated = JSON.parse(JSON.stringify(new Date()));
                                writeResponse = this.writeSmartLinksDetail(allOrganizationSmartLinkNodes);
    ...
    updateConnection(org_id, id, connections) {
        try {
            const clients = this.readClientsDetail();
            const allOrganizationSmartLinkNodes = this.readSmartLinksDetail();
            let writeResponse = false;
    ...
    deleteConnection(id, conn_id) {
        try {
            ...
            allOrganizationSmartLinkNodes.find((organizationSmartLinkNodes) => {
                organizationSmartLinkNodes.smartLink.find((smartLink) => {
                    if (id == smartLink.id) {
                        const dicomConnection = smartLink.connections.dicom.filter(dicom => dicom.conn_id != conn_id);
    ...
    downloadNodeConfiguration(org_id, id) {
        try {
            const clients = this.readClientsDetail();
                        ...
                        if (org_id == organizationSmartLink.org_id) {
                            organizationSmartLink.smartLink.find((smartLink) => {
                                if (id == smartLink.id) {
                                    ...
                                    const clientCert = this.getClientCertContent(org_id);
    ...
    nodeConfigurationFileName(org_id, id) {
        try {
            const allOrganizationSmartLinkNodes = this.readSmartLinksDetail();
            let smartLinkName;
            let org_name;
    ...
    getLatestInstaller() {
        try {
            const latestInstaller = this.readLatestInstaller();
            return {
                "status_code": 200,
                "message": null,
    ...
    getSoftwareDownloadUrl() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const result = yield aws_1.Aws.getUrlFromS3(keyString);
                return {
                    "status_code": 200,
                    "message": null,
    ...
    updateLatestInstaller(version, installer_name) {
        try {
            const latestInstaller = this.readLatestInstaller();
            latestInstaller.version = version;
            latestInstaller.installer_name = installer_name;
            const writeResponse = this.writeLatestInstaller(latestInstaller);
            if (writeResponse)
                return {
                    "status_code": 200,
                    "message": null,
    ...
    getCurrentSoftwareVersionInstalled(id) {
        try {
            const allOrganizationSmartLinkNodes = this.readSmartLinksDetail();
            let response;
    ...
    updateCurrentSoftwareVersionInstalled(org_id, id, version) {
        try {
            const clients = this.readClientsDetail();
            let writeResponse = false;
            clients.find((client) => {
    ...
    updateStatus(org_id, id, message) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const clients = this.readClientsDetail();
                let writeResponse = false;
    ...
    notifyUpdateConfiguration(requestData) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const notifySmartLinkResponse = yield this.notifySmartLink(requestData);
    ...
    notifySmartLink(requestData) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const axiosInstance = this.axiosInstance();
                let response = yield axiosInstance.post(process.env.EXO_MICROSERVICE_URL + "notify", requestData)
                    .then(response => {
    ...
    writeSmartLinksDetail(smartlinkDetails) {
        try {
    ...
    readLatestInstaller() {
        try {
            ...
            const latestInstaller = JSON.parse(lastestInstallerFile.toString());
    ...
    writeLatestInstaller(latestInstaller) {
        try {
    ...
    getRootCertificate() {
        try {
            const rootCertPath = `./cert/ca/ca.crt`;
...
exports.smartLinkUtlis = smartLinkUtlis;
//# sourceMappingURL=smartLinkUtils.js.map